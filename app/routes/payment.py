...
from app import db
from app.models import User, Payment, Plan
...
def plans():
    ...
    monthly_plan = Plan.query.filter_by(name='monthly').first()
    yearly_plan = Plan.query.filter_by(name='yearly').first()
...
def manage_subscription():
    ...
    # This condition is causing the problem - it's too strict
    # if not current_user.is_paid_user or not current_user.subscription_id:

    # Updated condition - only check if user is a paid user
    ...
    if payment_gateway == 'stripe':
        try:
            # Find the most recent payment to get customer ID
            latest_payment = Payment.query.filter_by(user_id=current_user.id).order_by(
                Payment.created_at.desc()
            ).first()

            if not latest_payment or not latest_payment.stripe_customer_id:
                flash('Unable to find your payment information', 'warning')
                return redirect(url_for('main.dashboard'))

            # Create a billing portal session
            session = stripe.billing_portal.Session.create(
                customer=latest_payment.stripe_customer_id,
        ...
        # Pass the current subscription info to the template
