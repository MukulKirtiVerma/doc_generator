    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            if file_size > current_app.config['MAX_CONTENT_LENGTH']:
                flash(
                    f'File too large. Maximum size is {current_app.config["MAX_CONTENT_LENGTH"] / 1024 / 1024:.1f} MB',
                    'danger')
                return redirect(request.url)
...
def process_document(document_id):
    ...
    if document.status == 'pending':
        ...
        try:
            ...
            ocr_text, anthropic_request_id = process_image_with_anthropic(file_path, current_user.anthropic_api_key)
            processing_time = time.time() - start_time

            # Store Anthropic request ID
            document.anthropic_request_id = anthropic_request_id

            # Generate the output document
            output_filename = f"{uuid.uuid4().hex}.{document.file_type}"
            output_path = os.path.join(current_app.config['UPLOAD_FOLDER'], output_filename)

            success, error_message = generate_document(ocr_text, document.file_type, output_path)
